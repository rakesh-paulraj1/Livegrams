  ...
  useEffect(() => {
    ...
    const connectWebSocket = async () => {
      try {
          ...
          try {
            ...
            if (data.record) {
              ...
              if (data.record.typeName === "instance_presence") {
                const userId =
                  (session?.user as { id?: string })?.id || "guest";
              ...
              const isUpdated =
                existing &&
                JSON.stringify(existing) !== JSON.stringify(data.record);
              if (isNew || isUpdated) {
            ...
            console.error(
              "[Client] Error processing WebSocket message:",
              error
            );
          }
        };
        ...
        const WS_URL = process.env.NEXT_PUBLIC_WS_URL;
        console.debug("[WS] connecting", { roomId, WS_URL });
